"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var mongodb_1 = require("mongodb");
var dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config();
var url = process.env.MONGODB_URI;
var client = new mongodb_1.MongoClient(url);
var dbName = "reactgraphdb";
var collectionName = "users";
var UserRepository = /** @class */ (function () {
    function UserRepository() {
    }
    UserRepository.connect = function () {
        return __awaiter(this, void 0, void 0, function () {
            var db;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, client.connect()];
                    case 1:
                        _a.sent();
                        db = client.db(dbName);
                        return [2 /*return*/, db.collection(collectionName)];
                }
            });
        });
    };
    UserRepository.findByMail = function (email) {
        return __awaiter(this, void 0, void 0, function () {
            var collection;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.findOne({ email: email })];
                    case 2: return [2 /*return*/, _a.sent()];
                }
            });
        });
    };
    UserRepository.findByUsername = function (username) {
        return __awaiter(this, void 0, void 0, function () {
            var collection;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.findOne({ username: username })];
                    case 2: return [2 /*return*/, _a.sent()];
                }
            });
        });
    };
    UserRepository.findByID = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, user;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.findOne({ _id: new mongodb_1.ObjectId(id) })];
                    case 2:
                        user = _a.sent();
                        return [2 /*return*/, user];
                }
            });
        });
    };
    UserRepository.getUserID = function (email) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, user;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.findOne({ email: email }, { projection: { _id: 1 } })];
                    case 2:
                        user = _a.sent();
                        if (!user)
                            return [2 /*return*/, null];
                        return [2 /*return*/, user._id.toString()];
                }
            });
        });
    };
    UserRepository.create = function (username, email, password) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, newUser, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        newUser = {
                            name: "",
                            username: username,
                            email: email,
                            password: password,
                            roles: [],
                            institution: "",
                            imgurl: "",
                        };
                        return [4 /*yield*/, collection.insertOne(newUser)];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, result.insertedId];
                }
            });
        });
    };
    UserRepository.delete = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.deleteOne({ _id: new mongodb_1.ObjectId(id) })];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, result.deletedCount > 0];
                }
            });
        });
    };
    UserRepository.updateName = function (name, id) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { name: name } })];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, result.modifiedCount > 0];
                }
            });
        });
    };
    UserRepository.updateUsername = function (username, id) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { username: username } })];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, result.modifiedCount > 0];
                }
            });
        });
    };
    UserRepository.updateInstitution = function (institution, id) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { institution: institution } })];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, result.modifiedCount > 0];
                }
            });
        });
    };
    UserRepository.updateMail = function (newMail, id) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { email: newMail } })];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, result.modifiedCount > 0];
                }
            });
        });
    };
    UserRepository.updatePassword = function (newPass, id) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { password: newPass } })];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, result.modifiedCount > 0];
                }
            });
        });
    };
    UserRepository.updateImgUrl = function (url, id) {
        return __awaiter(this, void 0, void 0, function () {
            var collection, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.connect()];
                    case 1:
                        collection = _a.sent();
                        return [4 /*yield*/, collection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { imgurl: url } })];
                    case 2:
                        result = _a.sent();
                        return [2 /*return*/, result.modifiedCount > 0];
                }
            });
        });
    };
    return UserRepository;
}());
exports.default = UserRepository;
